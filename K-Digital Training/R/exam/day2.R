...
mat1 <-matrix(chars) # row나 col값을 지정해주지 않으면 1열로 저장
mat1; dim(mat1)
matrix(chars, nrow=1)
matrix(chars, nrow=5)
matrix(chars, nrow=5, byrow=T) #byrow=T이면 행단위 저장 
matrix(chars, ncol=5)
...
mat1[1,1]
mat1[2,];mat1[,3]
mat1[1,1,drop=F] #drop=F는 매트릭스 형태 유지 
...
df <- data.frame(var1=c(4,3,8), 
                 var2=c(2,6,1))
str(df)
df$var_sum <- df$var1 + df$var2 # 새로운 변수(var_sum열) 추가 
df$var_mean <- df$var_sum/2 # 새로운 변수(var_mean열) 추가
df$result <- ifelse(df$var1>df$var2, 
                    "var1이 크다", "var1이 작다")

getwd() # ->get working directory의 약어 # setwd('xxx')
...
table(score$result) # 타입에 상관없이 무조건 개수를 세는 용도로 활용 가능 
...
emp[!is.na(emp$comm),]
emp[is.na(emp$comm),] # 커미션이 없는(NA) 직원 정보 출력

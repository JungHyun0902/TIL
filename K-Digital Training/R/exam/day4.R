...
#read file data
nums <- scan("data/sample_num.txt") # 수치
# encoding=코드셋명을 생략하고 읽으면 OS의 고유 코드셋 정보를 반영해서 읽는다.
...
df2 <- read.table("data/product_click.log", stringsAsFactors = T) # read.csv()는 ,로 구분된 데이터를 데이터 프레임으로 불러옴  # 데이터 구분을 공백이나 탭으로 줬을때 read.table을 사용 
str(df2)
head(df2)
summary(df2$V2)
